...
void dumpAddress(char *ipaddress)
{
  ...
  int i;
  for(i=SKIPINDEXINITIALIZER; i<IPADDRESSSIZE; i++)
  {
...
void dumpPrefix(char *ipaddress)
{
  ...
  printf("%d",(unsigned char)ipaddressnumberbyte);
  int i;
  for(i=SKIPINDEXINITIALIZER; i<IPADDRESSPREFIXSIZE; i++)
  {
    ipaddressnumberbyte = (int)(ipaddress[i]);
    printf(".%d",(unsigned char)ipaddressnumberbyte);
  }
...
{
  int i;
  for(i=INDEXINITIALIZER; i<listsize; i++)
  {
  ...
  int orglistfilenumlines, int maxlinelengthvalue)
{
  ...
  int i;
  for(i=INDEXINITIALIZER; i<orglistfilenumlines; i++)
  {
    //read ip address byte by byte
    int j;
    for(j=INDEXINITIALIZER; j<IPADDRESSPREFIXSIZE; j++)
    {
    ...
    int k;
    for(k=INDEXINITIALIZER; k<IPADDRESSPREFIXSIZE; k++)
...
int main(int argc, char *argv[])
{
  int flags[NUMFLAGS];
  int i;
  for(i=INDEXINITIALIZER; i<NUMFLAGS; i++)
    flags[i] = FALSE;
  int input;
  ...
  if(flags[FLAGC] == TRUE)
  {
    ...
    int i;
    for(i=INDEXINITIALIZER; i<*maxlinelength; i++)
    {
